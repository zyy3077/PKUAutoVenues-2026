...
项目提供了一个本地 Web UI，用于填写预约参数、启动后台任务并查看实时日志。它复用现有的 `main.py`，不需要额外的前端依赖：
...
然后打开 <http://127.0.0.1:8000>。填写场馆、目标日期、开始时间（例如 `20:00/2`）、优先场地和预约类型后，点击“创建预约任务”。关闭网页不会停止任务；任务状态和最近 300 行日志会在页面中更新。默认只监听本机地址，若要从其他设备访问，应通过 SSH 端口转发，而不要直接暴露到公网。
