...
def start_task(payload: dict) -> dict:
    ...
    if venue in ("108", "二体"):
        command += ["--court-type", str(payload.get("court_type", "auto"))]
...
</style><body><div class=shell><header class=hero><div><div class=eyebrow>PKU AUTO VENUES</div><h1>预约控制台</h1><p>配置一次预约任务，实时掌握抢场进度。</p></div><div class=pill>● 本地运行</div></header><div class=grid><section class=card><h2>创建预约任务</h2><p class=hint>任务会在后台运行，关闭页面不会中断预约。</p><form id=f><div class=row><label>场馆<select name=venue required><option value=60>邱德拔羽毛球馆</option><option value=86>五四羽毛球馆</option><option value=68>邱德拔篮球馆</option><option value=82>五四篮球场</option><option value=108>二体篮球场</option></select></label><label>预约日期<input name=date type=date required></label></div><fieldset class=time-picker><legend>目标时段</legend><p class=hint>按列表顺序尝试，每项可预约连续 1 或 2 小时。</p><div id=time-options class=time-options></div><button type=button class="primary add-time" id=add-time>＋ 添加目标时间</button></fieldset><label id=court-type-field hidden>预约类型<select name=court_type disabled><option value=auto>未指定</option><option value=half>半场</option><option value=full>整场</option></select></label><fieldset class=time-picker><legend>优先场地</legend><p class=hint>可多选，按勾选顺序优先尝试；不选则随机选择。</p><div id=space-options class=space-options></div><p id=space-order class=hint aria-live=polite></p></fieldset><label class=check><input name=skip_pay type=checkbox>跳过自动支付</label><button class=primary id=submit>启动预约任务</button></form></section><section class="card log-card"><div class=loghead><div><h2>运行状态</h2><div id=s class=status><i class=dot></i><span>等待创建任务</span></div></div><span id=task></span><button class=primary type=button id=kill onclick="stop()" disabled>中止任务</button></div><div id=reservation-result class=result-card role=status aria-live=polite hidden><h3 id=result-title></h3><p id=result-message></p></div><div id=empty class=empty><div><strong>还没有运行日志</strong><span>创建任务后，验证码、查询和提交进度会显示在这里</span></div></div><pre id=log class=logbox hidden></pre></section></div></div>
<script>
...
function updateSpaces(){
 ...
 const halves=venue==='82'||venue==='68'||(venue==='108'&&courtTypeSelect.value==='half');
